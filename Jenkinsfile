...
#                                             # Извлекаем namespace из PKI пути (первая часть до первого /)
                                            # PKI_PATH="${params.SBERCA_CERT_KV}"
                                            # PKI_NAMESPACE=\$(echo "\$PKI_PATH" | cut -d'/' -f1)
                                            
#                                             # echo "[CERTS] KAE namespace: ${env.KAE}" >&2
                                            # echo "[CERTS] PKI namespace (extracted): \$PKI_NAMESPACE" >&2
                                            
#                                             # Пробуем аутентификацию с KAE namespace
...
#                                             # ВАРИАНТ 1: Без namespace header (PKI путь уже полный)
                                            # echo "[CERTS] Attempt 1: POST without namespace header" >&2
                                            # RESPONSE=\$(curl -s -w "\nHTTP_CODE:%{http_code}" -X POST -H "X-Vault-Token: \$VAULT_TOKEN" \
                                                # "https://${params.SEC_MAN_ADDR}/v1/\$PKI_PATH" \
                                                # -d '{"common_name":"${fqdn}","email":"${email}","alt_names":"${fqdn}","ttl":"${ttl}"}')
                                            # HTTP_CODE=\$(echo "\$RESPONSE" | grep "HTTP_CODE:" | cut -d: -f2)
                                            # RESPONSE=\$(echo "\$RESPONSE" | sed '/HTTP_CODE:/d')
                                            
#                                             # echo "[CERTS] HTTP Code: \$HTTP_CODE" >&2
                                            
#                                             # Если permission denied, пробуем другие варианты
                                            # if echo "\$RESPONSE" | grep -q "permission denied"; then
                                                # echo "[CERTS] Attempt 1 failed, trying with PKI namespace header..." >&2
