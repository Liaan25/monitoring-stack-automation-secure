...
ensure_user_in_va_read_group() {
    ...
    echo "[VA-READ] Проверка: состоит ли $user в группе $va_read_group..."
    log_debug "Checking if $user is already in $va_read_group"
    
    if id "$user" 2>/dev/null | grep -q "$va_read_group"; then
        echo "[VA-READ] ✅ Пользователь $user УЖЕ СОСТОИТ в группе $va_read_group"
    ...
    echo "[VA-READ] ⚠️  Пользователь $user НЕ в группе $va_read_group, создаем RLM задачу..."
...
ensure_user_in_va_start_group() {
    ...
    # ПРОВЕРКА: Может пользователь уже в группе?
    echo "[VA-START] Проверка: состоит ли $user в группе $va_start_group..."
    log_debug "Checking if $user is already in $va_start_group"
    
    if id "$user" 2>/dev/null | grep -q "$va_start_group"; then
        echo "[VA-START] ✅ Пользователь $user УЖЕ СОСТОИТ в группе $va_start_group"
    ...
    echo "[VA-START] ⚠️  Пользователь $user НЕ в группе $va_start_group, создаем RLM задачу..."
...
check_vault_agent_preflight_access() {
    ...
    echo "[VAULT-PREFLIGHT] Проверка прав для legacy vault-agent flow"
    echo "[VAULT-PREFLIGHT] Текущий пользователь: $current_user"
    echo "[VAULT-PREFLIGHT] KAE=${KAE:-<unknown>}"
    echo "[VAULT-PREFLIGHT] Ожидаемый источник agent.hcl: $VAULT_AGENT_HCL"
    echo "[VAULT-PREFLIGHT] ========================================"

    # Проверка membership - изменения групп применяются только в новой сессии.
    if id -nG "$current_user" 2>/dev/null | tr ' ' '\n' | grep -Fxq "${KAE}-lnx-va-start"; then
        echo "[VAULT-PREFLIGHT] ✅ Пользователь уже в группе ${KAE}-lnx-va-start"
    else
        echo "[VAULT-PREFLIGHT] ⚠️  Пользователь НЕ в группе ${KAE}-lnx-va-start (нужна новая сессия после добавления)"
    fi
    ...
    check_sudo_rule() {
        ...
        if sudo -n -l -- "$@" >/dev/null 2>&1; then
            echo "[VAULT-PREFLIGHT] ✅ sudo rule есть: $label"
        else
            echo "[VAULT-PREFLIGHT] ❌ sudo rule отсутствует: $label"
            echo "[VAULT-PREFLIGHT]    EXPECTED: ALL=(root) NOEXEC: NOPASSWD: $*"
            missing=1
        fi
    }
    ...
    if [[ $missing -ne 0 ]]; then
        echo "[VAULT-PREFLIGHT] ========================================"
        echo "[VAULT-PREFLIGHT] ❌ Обнаружены недостающие права/правила."
        echo "[VAULT-PREFLIGHT] Подсказка: проверьте, что KAE в sudoers совпадает с текущим (${KAE})."
        echo "[VAULT-PREFLIGHT] Частая ошибка: правило для /home/<другой-KAE>-lnx-mon_ci/.../agent.hcl."
        echo "[VAULT-PREFLIGHT] Диагностика: sudo -n -l"
        echo "[VAULT-PREFLIGHT] ========================================"
    else
        echo "[VAULT-PREFLIGHT] ✅ Проверка прав пройдена"
    fi
}
...
setup_vault_config() {
   ...
   echo "[VAULT-CONFIG] Извлечение секретов..."
   ...
   if [[ -x "$WRAPPERS_DIR/secrets-manager-wrapper_launcher.sh" ]]; then
       echo "[VAULT-CONFIG] Используем secrets-manager-wrapper..."
       ...
       if [[ "$secrets_extracted" == "true" ]]; then
           echo "[VAULT-CONFIG] ✅ Секреты успешно извлечены через wrapper"
       else
           echo "[VAULT-CONFIG] ⚠️  Секреты НЕ извлечены через wrapper"
       fi
   else
       echo "[VAULT-CONFIG] ⚠️  secrets-manager-wrapper_launcher.sh не найден или не исполняемый"
   ...
   echo "[VAULT-CONFIG] ========================================"
   echo "[VAULT-CONFIG] Извлечение секретов в /tmp/ через jq"
   echo "[VAULT-CONFIG] ========================================"
   log_debug "========================================"
   ...
   echo "[VAULT-CONFIG] Временные файлы:"
   echo "[VAULT-CONFIG]   TMP_ROLE_ID=$TMP_ROLE_ID"
   echo "[VAULT-CONFIG]   TMP_SECRET_ID=$TMP_SECRET_ID"
   ...
   if command -v jq >/dev/null 2>&1 && [[ -f "$cred_json_path" ]]; then
       echo "[VAULT-CONFIG] jq доступен, извлекаем секреты..."
       ...
       if jq -e '.["vault-agent"].role_id' "$cred_json_path" >/dev/null 2>&1 && \
          jq -e '.["vault-agent"].secret_id' "$cred_json_path" >/dev/null 2>&1; then
           echo "[VAULT-CONFIG] ✅ Секреты найдены в JSON файле"
           ...
           if jq -r '.["vault-agent"].role_id' "$cred_json_path" > "$TMP_ROLE_ID" 2>/dev/null && \
              jq -r '.["vault-agent"].secret_id' "$cred_json_path" > "$TMP_SECRET_ID" 2>/dev/null; then
               ...
               echo "[VAULT-CONFIG] Проверка извлеченных файлов..."
               ...
               if [[ -s "$TMP_ROLE_ID" && -s "$TMP_SECRET_ID" ]]; then
                   ...
                   echo "[VAULT-CONFIG] ✅ Секреты успешно извлечены в /tmp/"
                   echo "[VAULT-CONFIG]   role_id: $role_id_size байт"
                   echo "[VAULT-CONFIG]   secret_id: $secret_id_size байт"
                   ...
                   echo "[VAULT-CONFIG] Копирование в user-space для справки..."
                   if cp "$TMP_ROLE_ID" "$VAULT_ROLE_ID_FILE" 2>/dev/null && \
                      cp "$TMP_SECRET_ID" "$VAULT_SECRET_ID_FILE" 2>/dev/null; then
                       ...
                       echo "[VAULT-CONFIG] ✅ Скопировано в user-space"
                       log_debug "✅ Copied to user-space"
                   else
                       echo "[VAULT-CONFIG] ⚠️  Не удалось скопировать в user-space (не критично)"
                       log_debug "⚠️  Failed to copy to user-space (not critical)"
                   fi
               else
                   echo "[VAULT-CONFIG] ❌ Извлеченные секреты пустые"
                   log_debug "❌ Extracted secrets are empty"
                   secrets_extracted=false
               fi
           else
               echo "[VAULT-CONFIG] ❌ Ошибка при извлечении секретов через jq"
               log_debug "❌ Error extracting secrets via jq"
               secrets_extracted=false
           fi
       else
           echo "[VAULT-CONFIG] ❌ Секреты НЕ найдены в JSON файле"
           echo "[VAULT-CONFIG] Проверьте наличие полей: .['vault-agent'].role_id и .['vault-agent'].secret_id"
           log_debug "❌ Secrets NOT found in JSON file"
           secrets_extracted=false
       fi
   else
       echo "[VAULT-CONFIG] ❌ jq не найден или файл недоступен"
       log_debug "❌ jq not found or file inaccessible"
       secrets_extracted=false
   fi
    
   if [[ "$secrets_extracted" != "true" ]]; then
       echo "[VAULT-CONFIG] ⚠️  Vault-agent НЕ сможет аутентифицироваться!"
   ...
   echo "[VAULT-CONFIG] ========================================"
   echo "[VAULT-CONFIG] После извлечения секретов"
   echo "[VAULT-CONFIG] ========================================"
   log_debug "========================================"
   log_debug "POST-SECRETS: Setting permissions"
   log_debug "========================================"
    
   echo "[VAULT-CONFIG] Установка прав на файлы секретов..."
   echo "[VAULT-CONFIG] VAULT_ROLE_ID_FILE=$VAULT_ROLE_ID_FILE"
   echo "[VAULT-CONFIG] VAULT_SECRET_ID_FILE=$VAULT_SECRET_ID_FILE"
   ...
   if chmod 640 "$VAULT_ROLE_ID_FILE" "$VAULT_SECRET_ID_FILE" 2>/dev/null; then
       echo "[VAULT-CONFIG] ✅ chmod 640 успешен"
       log_debug "✅ chmod 640 successful"
   else
       echo "[VAULT-CONFIG] ⚠️  chmod 640 failed (не критично)"
       log_debug "⚠️  chmod 640 failed"
   fi
    
    # SECURE EDITION: Пропускаем chown операции (нет доступа к /opt/, не нужны в user-space)
   echo "[VAULT-CONFIG] SECURE EDITION: Пропускаем chown операции (не нужны в user-space)"
   ...
   echo "[VAULT-CONFIG] Создание vault-agent.conf..."
   log_debug "Creating vault-agent.conf"

   echo "[VAULT-CONFIG] Начинается блок генерации vault-agent.conf..."
   ...
   echo "[VAULT-CONFIG] Создание agent.hcl в user-space..."
   ...
   echo "[VAULT-CONFIG] Генерация полного agent.hcl..."
...
SYS_EOF
   ...
   echo "[VAULT-CONFIG] ✅ agent.hcl создан в: $VAULT_AGENT_HCL"
   log_debug "✅ agent.hcl created at $VAULT_AGENT_HCL"
   print_success "agent.hcl успешно создан"
   
   echo "[VAULT-CONFIG] ========================================"
   echo "[VAULT-CONFIG] agent.hcl будет скопирован в /opt/vault/conf/ через sudo"
   echo "[VAULT-CONFIG] ========================================"
   ...
   echo "[VAULT-CONFIG] ========================================"
   echo "[VAULT-CONFIG] Копирование credentials в /opt/vault/conf/"
   echo "[VAULT-CONFIG] ========================================"
   log_debug "========================================"
   ...
   if [[ "$secrets_extracted" == "true" && -s "$TMP_ROLE_ID" && -s "$TMP_SECRET_ID" ]]; then
       ...
       if [[ ! -d "/opt/vault/certs" ]]; then
           ...
           if sudo -n /usr/bin/mkdir -p /opt/vault/certs 2>/dev/null; then
               ...
               if sudo -n /usr/bin/chown "${KAE}-lnx-va-start:${KAE}-lnx-va-read" /opt/vault/certs 2>/dev/null; then
                   echo "[VAULT-CONFIG] ✅ Владелец установлен: ${KAE}-lnx-va-start:${KAE}-lnx-va-read"
                   log_debug "✅ Owner set for /opt/vault/certs/"
               fi
                
               if sudo -n /usr/bin/chmod 750 /opt/vault/certs 2>/dev/null; then
                   echo "[VAULT-CONFIG] ✅ Права установлены: 750"
                   log_debug "✅ Permissions set for /opt/vault/certs/"
               fi
           else
               echo "[VAULT-CONFIG] ⚠️  Не удалось создать /opt/vault/certs/ (требуется sudo)"
           ...
           echo "[VAULT-CONFIG] ✅ /opt/vault/certs/ уже существует"
           log_debug "✅ /opt/vault/certs/ already exists"
       fi
        
        # 2. Копируем role_id.txt из /tmp/ в /opt/vault/conf/
       echo "[VAULT-CONFIG] Копирование role_id.txt..."
       ...
       echo "[VAULT-CONFIG] Копирование secret_id.txt..."
       ...
       if sudo -n /usr/bin/cp "$TMP_SECRET_ID" /opt/vault/conf/secret_id.txt 2>/dev/null; then
           echo "[VAULT-CONFIG] ✅ secret_id.txt скопирован"
       ...
       echo "[VAULT-CONFIG] Копирование agent.hcl..."
       ...
       if [[ -f "$VAULT_AGENT_HCL" ]]; then
           ...
           if [[ -w "/opt/vault/conf/" ]]; then
               if cp "$VAULT_AGENT_HCL" "$target_agent_hcl" 2>/dev/null; then
                   ...
                   echo "[VAULT-CONFIG] ✅ agent.hcl скопирован без sudo (через права директории)"
                   log_debug "✅ agent.hcl copied without sudo"
                   chmod 640 "$target_agent_hcl" 2>/dev/null || true
               else
                   echo "[VAULT-CONFIG] ⚠️  Копирование без sudo не удалось, пробуем через sudo..."
                   log_debug "⚠️  Non-sudo copy failed, will try sudo fallback"
               fi
           else
               echo "[VAULT-CONFIG] ⚠️  Нет прав записи в /opt/vault/conf/, пробуем через sudo..."
           ...
           if [[ "$copied_agent_hcl" != "true" ]]; then
               if sudo -n /usr/bin/cp "$VAULT_AGENT_HCL" "$target_agent_hcl" 2>/dev/null; then
                   copied_agent_hcl=true
                   echo "[VAULT-CONFIG] ✅ agent.hcl скопирован через sudo"
                   ...
                   echo "[VAULT-CONFIG] ✅ agent.hcl права установлены (640)"
                       log_debug "✅ agent.hcl chmod 640 successful (sudo)"
               fi
           else
                   echo "[VAULT-CONFIG] ❌ Не удалось скопировать agent.hcl ни без sudo, ни через sudo"
                   echo "[VAULT-CONFIG] EXPECTED sudo rule: ALL=(root) NOEXEC: NOPASSWD: /usr/bin/cp $VAULT_AGENT_HCL /opt/vault/conf/agent.hcl"
           ...
           if [[ -f "$target_agent_hcl" && -s "$target_agent_hcl" ]]; then
               echo "[VAULT-CONFIG] ✅ agent.hcl присутствует в /opt/vault/conf/"
               log_debug "✅ agent.hcl final check passed"
           else
               echo "[VAULT-CONFIG] ❌ agent.hcl отсутствует в /opt/vault/conf/ после попыток копирования"
               log_debug "❌ agent.hcl missing after copy attempts"
           fi
       else
           echo "[VAULT-CONFIG] ⚠️  agent.hcl не найден в: $VAULT_AGENT_HCL"
           log_debug "⚠️  agent.hcl not found"
       fi
        
        # 4. Очищаем временные файлы из /tmp/ (важно для безопасности!)
       echo "[VAULT-CONFIG] Очистка временных файлов из /tmp/..."
       log_debug "Cleaning up temporary files from /tmp/"
        
       if rm -f "$TMP_ROLE_ID" "$TMP_SECRET_ID" 2>/dev/null; then
           echo "[VAULT-CONFIG] ✅ Временные файлы удалены"
           log_debug "✅ Temporary files removed"
       else
           echo "[VAULT-CONFIG] ⚠️  Не удалось удалить временные файлы"
           log_debug "⚠️  Failed to remove temporary files"
       fi
        
       echo "[VAULT-CONFIG] ✅ Credentials успешно установлены в /opt/vault/conf/"
       log_debug "✅ Credentials successfully installed"
   else
       echo "[VAULT-CONFIG] ⚠️  Секреты не извлечены или файлы пустые, пропускаем копирование"
    ...
    # ============================================================
   echo "[VAULT-CONFIG] Применение конфигурации к vault-agent..."
   ...
   if [[ -w "/opt/vault/conf/" ]]; then
           ...
           if sudo -n /usr/bin/systemctl restart vault-agent 2>/dev/null; then
               ...
               if sudo -n /usr/bin/systemctl is-active vault-agent >/dev/null 2>&1; then
                   ...
                   if sudo -n /usr/bin/systemctl status vault-agent --no-pager 2>&1 | tee -a "$DIAGNOSTIC_RLM_LOG"; then
                       echo "[VAULT-CONFIG] Статус vault-agent записан в лог"
                   fi
               fi
           else
               echo "[VAULT-CONFIG] ❌ Не удалось перезапустить vault-agent (требуется sudo)"
       ...
       if ensure_user_in_va_start_group "$current_user"; then
           ...
           echo "[VAULT-CONFIG] ❌ Не удалось добавить в группу va-start"
   ...
   echo "[VAULT-CONFIG] ========================================"
   echo "[VAULT-CONFIG] ✅ setup_vault_config ЗАВЕРШЕНА УСПЕШНО"
   echo "[VAULT-CONFIG] ========================================"
   log_debug "========================================"
...
copy_certs_to_user_dirs() {
    echo "[CERTS-COPY] ========================================"
    echo "[CERTS-COPY] Копирование сертификатов (Secure Edition)"
    echo "[CERTS-COPY] ========================================"
    log_debug "========================================"
    ...
    echo "[CERTS-COPY] Источники сертификатов (user-space):"
    echo "[CERTS-COPY]   vault_bundle=$vault_bundle"
    echo "[CERTS-COPY]   grafana_client_pem=$grafana_client_pem"
    ...
    if [[ ! -f "$vault_bundle" ]]; then
        echo "[CERTS-COPY] ❌ vault_bundle не найден: $vault_bundle"
    ...
    if [[ ! -s "$vault_bundle" ]]; then
        echo "[CERTS-COPY] ❌ vault_bundle пустой: $vault_bundle"
    ...
    if ! grep -q "BEGIN.*PRIVATE KEY" "$vault_bundle"; then
        echo "[CERTS-COPY] ⚠️  vault_bundle не содержит приватный ключ (нет BEGIN PRIVATE KEY)"
    ...
    echo "[CERTS-COPY] ✅ Создана директория: $harvest_cert_dir"
    log_debug "✅ Created directory: $harvest_cert_dir"
    
    echo "[CERTS-COPY] Поиск приватного ключа..."
    ...
    for key_file in "${possible_key_files[@]}"; do
        if [[ -f "$key_file" && -r "$key_file" ]]; then
            echo "[CERTS-COPY] ✅ Найден приватный ключ: $key_file"
            log_debug "✅ Found private key: $key_file"
            cp "$key_file" "$harvest_cert_dir/harvest.key" || {
                echo "[CERTS-COPY] ❌ Не удалось скопировать ключ"
    ...
    if [[ "$private_key_found" == "false" ]]; then
        ...
        if openssl pkey -in "$vault_bundle" -out "$harvest_cert_dir/harvest.key" 2>/dev/null; then
            echo "[CERTS-COPY] ✅ Ключ извлечен из bundle"
            log_debug "✅ Key extracted from bundle"
            private_key_found=true
            private_key_source="bundle"
        else
            echo "[CERTS-COPY] ❌ Не удалось найти или извлечь приватный ключ"
    ...
    echo "[CERTS-COPY] Извлечение сертификата из bundle..."
    ...
    openssl crl2pkcs7 -nocrl -certfile "$vault_bundle" | openssl pkcs7 -print_certs -out "$harvest_cert_dir/harvest.crt" 2>/dev/null || {
        echo "[CERTS-COPY] ❌ Не удалось извлечь сертификат"
        log_debug "❌ Failed to extract certificate"
        print_error "Не удалось извлечь сертификат из $vault_bundle"
        exit 1
    }
    echo "[CERTS-COPY] ✅ Извлечены harvest.key и harvest.crt"
    echo "[CERTS-COPY]   Источник ключа: $private_key_source"
    ...
    echo "[CERTS-COPY] ✅ Harvest сертификаты: $harvest_cert_dir/harvest.{crt,key}"
    log_debug "✅ Harvest certificates: $harvest_cert_dir/harvest.{crt,key}"
    
    # ========================================
    # 2. Grafana сертификаты
    # ========================================
    echo "[CERTS-COPY] 2/3: Обработка сертификатов для Grafana..."
    log_debug "Processing Grafana certificates..."
    
    mkdir -p "$GRAFANA_USER_CERTS_DIR" || {
        echo "[CERTS-COPY] ❌ Не удалось создать $GRAFANA_USER_CERTS_DIR"
    ...
    echo "[CERTS-COPY] ✅ Создана директория: $GRAFANA_USER_CERTS_DIR"
    log_debug "✅ Created directory: $GRAFANA_USER_CERTS_DIR"
    
    echo "[CERTS-COPY] Копирование ключа и сертификата для Grafana..."
    ...
    if [[ -f "$harvest_cert_dir/harvest.key" ]]; then
        cp "$harvest_cert_dir/harvest.key" "$GRAFANA_USER_CERTS_DIR/key.key" || {
            echo "[CERTS-COPY] ❌ Не удалось скопировать ключ для Grafana"
            log_debug "❌ Failed to copy key for Grafana"
            exit 1
        }
    else
        echo "[CERTS-COPY] ❌ Ключ Harvest не найден: $harvest_cert_dir/harvest.key"
    ...
    openssl crl2pkcs7 -nocrl -certfile "$vault_bundle" | openssl pkcs7 -print_certs -out "$GRAFANA_USER_CERTS_DIR/crt.crt" 2>/dev/null || {
        echo "[CERTS-COPY] ❌ Не удалось извлечь сертификат для Grafana"
        log_debug "❌ Failed to extract certificate for Grafana"
        exit 1
    }
    echo "[CERTS-COPY] ✅ Извлечены key.key и crt.crt для Grafana"
    ...
    echo "[CERTS-COPY] ✅ Grafana сертификаты: $GRAFANA_USER_CERTS_DIR/{crt.crt,key.key}"
    ...
    if [[ -f "$grafana_client_pem" ]]; then
        echo "[CERTS-COPY] Обработка Grafana client certificate..."
        ...
        echo "[CERTS-COPY] ✅ Grafana client cert обработан"
        log_debug "✅ Grafana client cert processed"
    else
        echo "[CERTS-COPY] ⚠️  Grafana client cert не найден: $grafana_client_pem"
    ...
    # ========================================
    echo "[CERTS-COPY] 3/3: Обработка сертификатов для Prometheus..."
    log_debug "Processing Prometheus certificates..."
    
    mkdir -p "$PROMETHEUS_USER_CERTS_DIR" || {
        echo "[CERTS-COPY] ❌ Не удалось создать $PROMETHEUS_USER_CERTS_DIR"
    ...
    echo "[CERTS-COPY] ✅ Создана директория: $PROMETHEUS_USER_CERTS_DIR"
    log_debug "✅ Created directory: $PROMETHEUS_USER_CERTS_DIR"
    
    echo "[CERTS-COPY] Копирование ключа и сертификата для Prometheus..."
    log_debug "Copying key and cert for Prometheus"
    
    # Копируем тот же ключ что использовали для Harvest
    if [[ -f "$harvest_cert_dir/harvest.key" ]]; then
        cp "$harvest_cert_dir/harvest.key" "$PROMETHEUS_USER_CERTS_DIR/server.key" || {
            echo "[CERTS-COPY] ❌ Не удалось скопировать ключ для Prometheus"
    ...
    openssl crl2pkcs7 -nocrl -certfile "$vault_bundle" | openssl pkcs7 -print_certs -out "$PROMETHEUS_USER_CERTS_DIR/server.crt" 2>/dev/null || {
        echo "[CERTS-COPY] ❌ Не удалось извлечь сертификат для Prometheus"
        log_debug "❌ Failed to extract certificate for Prometheus"
        exit 1
    }
    echo "[CERTS-COPY] ✅ Извлечены server.key и server.crt для Prometheus"
    ...
    echo "[CERTS-COPY] ✅ Prometheus сертификаты: $PROMETHEUS_USER_CERTS_DIR/{server.crt,server.key}"
    ...
    if [[ -f "$ca_chain" ]]; then
        echo "[CERTS-COPY] Копирование CA chain во все директории..."
        ...
        echo "[CERTS-COPY] ✅ CA chain скопирован"
        log_debug "✅ CA chain copied"
    else
        echo "[CERTS-COPY] ⚠️  CA chain не найден: $ca_chain"
        log_debug "⚠️  CA chain not found: $ca_chain"
    fi
    
    echo "[CERTS-COPY] ========================================"
    echo "[CERTS-COPY] ✅ Все сертификаты скопированы в user-space"
    echo "[CERTS-COPY] ========================================"
    log_debug "========================================"
...
EOF
    ...
    echo "[DEBUG-SYSTEMD] Юниты созданы в: $user_systemd_dir"
    echo "[DEBUG-SYSTEMD] Prometheus unit: $prom_unit"
...
EOF
    ...
    # Логируем для отладки
    echo "[DEBUG-GRAFANA] Конфиг создан: $GRAFANA_USER_CONFIG_DIR/grafana.ini"
    echo "[DEBUG-GRAFANA] Данные: $GRAFANA_USER_DATA_DIR"
    echo "[DEBUG-GRAFANA] Логи: $GRAFANA_USER_LOGS_DIR"
    echo "[DEBUG-GRAFANA] Сертификаты: $GRAFANA_USER_CERTS_DIR/"
}
...
EOF
    ...
    # Логируем для отладки
    echo "[DEBUG-PROMETHEUS] Конфиг: $PROMETHEUS_USER_CONFIG_DIR/"
    echo "[DEBUG-PROMETHEUS] Данные: $PROMETHEUS_USER_DATA_DIR"
    echo "[DEBUG-PROMETHEUS] Сертификаты: $PROMETHEUS_USER_CERTS_DIR/"
}
...
create_rlm_install_tasks() {
    ...
    echo "[RLM-INSTALL] Настройка PATH для Harvest..."
    ...
    if [[ -f "/opt/harvest/bin/harvest" ]]; then
        echo "[RLM-INSTALL] ✅ Найден harvest: /opt/harvest/bin/harvest"
        ...
        echo "[RLM-INSTALL] ✅ Найден harvest: /opt/harvest/harvest"
        ...
        echo "[RLM-INSTALL] ⚠️  harvest не найден в стандартных путях"
    ...
    export PATH=$PATH:/opt/harvest/bin:/opt/harvest
    echo "[RLM-INSTALL] PATH обновлен для текущей сессии: $PATH"
    log_debug "PATH updated for current session"
    print_success "PATH настроен для Harvest (в рамках текущей сессии)"
    
    echo "[RLM-INSTALL] ========================================"
    echo "[RLM-INSTALL] ✅ create_rlm_install_tasks ЗАВЕРШЕНА"
    echo "[RLM-INSTALL] ========================================"
...
setup_certificates_after_install() {
    echo "[CERTS] ========================================"
    echo "[CERTS] Настройка сертификатов (Secure Edition)"
    echo "[CERTS] ========================================"
    log_debug "========================================"
    ...
    echo "[CERTS] ========================================"
    echo "[CERTS] Проверка источников сертификатов..."
    echo "[CERTS] ========================================"
    echo "[CERTS] Системный путь (vault-agent): $system_vault_bundle"
    echo "[CERTS] User-space путь: $userspace_vault_bundle"
    echo "[CERTS] Альтернативные пути:"
    echo "[CERTS]   VAULT_CRT_FILE=${VAULT_CRT_FILE:-<не задан>}"
    echo "[CERTS]   VAULT_KEY_FILE=${VAULT_KEY_FILE:-<не задан>}"
    ...
    if [[ -f "$system_vault_bundle" ]]; then
        echo "[CERTS] ✅ Найден системный vault bundle: $system_vault_bundle"
        ...
        if [[ -r "$system_vault_bundle" ]]; then
            ...
            if ensure_user_in_va_read_group "$USER"; then
                ...
                if id "$sys_user" 2>/dev/null | grep -q "${KAE}-lnx-va-read"; then
                    echo "[CERTS] 🔧 Попытка копирования через sudo -u $sys_user..."
                ...
                # Финальная проверка
                if [[ "$copy_success" == false ]]; then
                    echo "[CERTS] ❌ Все способы копирования не сработали"
                ...
                if id "$sys_user" | grep -q "${KAE}-lnx-va-read"; then
                    ...
                    echo "[CERTS] Копирование от имени $sys_user..."
                    ...
                    if sudo -n -u "$sys_user" cp "$system_vault_bundle" "$userspace_vault_bundle" 2>/dev/null; then
                        ...
                        echo "[CERTS] ⚠️  Не удалось скопировать через sudo -u $sys_user"
                    ...
                    echo "[CERTS] ❌ $sys_user НЕ состоит в группе va-read"
        ...
        if [[ -f "/opt/vault/certs/ca_chain.crt" ]]; then
            ...
            if [[ -f "$VAULT_CERTS_DIR/ca_chain.crt" ]]; then
                echo "[CERTS] ✅ CA chain скопирован"
                log_debug "✅ CA chain copied"
            else
                echo "[CERTS] ⚠️  CA chain не скопирован (нет прав)"
        ...
        if [[ -f "/opt/vault/certs/grafana-client.pem" ]]; then
            ...
            if [[ -f "$MONITORING_CERTS_DIR/grafana/grafana-client.pem" ]]; then
                echo "[CERTS] ✅ Grafana client cert скопирован"
                log_debug "✅ Grafana client cert copied"
            else
                echo "[CERTS] ⚠️  Grafana client cert не скопирован (нет прав)"
                log_debug "⚠️  Grafana client cert not copied (no permissions)"
            fi
        fi
        
        echo "[CERTS] ✅ Сертификаты скопированы в user-space"
        ...
        if [[ -f "$PROMETHEUS_USER_CERTS_DIR/server.crt" && -f "$PROMETHEUS_USER_CERTS_DIR/server.key" ]]; then
            echo "[CERTS] ✅ Prometheus сертификаты присутствуют в $PROMETHEUS_USER_CERTS_DIR"
            ...
            echo "[CERTS] ❌ Отсутствуют Prometheus сертификаты в $PROMETHEUS_USER_CERTS_DIR"
    ...
    # Приоритет 2: уже скопированный user-space bundle
        ...
        echo "[CERTS] ✅ Найден user-space vault bundle: $userspace_vault_bundle"
    ...
    # Приоритет 3: отдельные .crt/.key файлы
        ...
        echo "[CERTS] ✅ Найдена пара сертификатов: $VAULT_CRT_FILE, $VAULT_KEY_FILE"
        ...
        echo "[CERTS] ❌ Сертификаты не найдены ни в одном из путей!"
        ...
        echo "[CERTS] Проверка содержимого /opt/vault/certs/:"
        ls -la /opt/vault/certs/ 2>&1 || echo "[CERTS] Директория не существует или недоступна"
        exit 1
    fi
    
    echo "[CERTS] ========================================"
    echo "[CERTS] ✅ setup_certificates_after_install ЗАВЕРШЕНА"
    echo "[CERTS] ========================================"
    log_debug "========================================"
...
EOF
    ...
    # Логируем для отладки
    echo "[DEBUG-HARVEST] Конфиг Unix: $HARVEST_UNIX_CONFIG"
    echo "[DEBUG-HARVEST] Конфиг NetApp: $HARVEST_NETAPP_CONFIG"
    echo "[DEBUG-HARVEST] Сертификаты: $HARVEST_USER_CERTS_DIR/"
    if [[ -f "$HARVEST_USER_CERTS_DIR/harvest.crt" && -f "$HARVEST_PKCS8_KEY" ]]; then
        echo "[DEBUG-HARVEST] ✅ Найдены harvest.crt и harvest_pkcs8.key"
    else
        echo "[DEBUG-HARVEST] ❌ Нет пары harvest.crt/harvest_pkcs8.key в $HARVEST_USER_CERTS_DIR"
    fi
}
...
configure_services() {
    ...
    echo "[CERTS-SVC-DIAG] userspace_pair: $userspace_pair_ok (crt=$VAULT_CRT_FILE, key=$VAULT_KEY_FILE)"
    echo "[CERTS-SVC-DIAG] userspace_bundle: $userspace_bundle_ok ($userspace_bundle)"
    echo "[CERTS-SVC-DIAG] userspace_ca_chain: $userspace_ca_ok ($userspace_ca_chain)"
    echo "[CERTS-SVC-DIAG] system_bundle: $system_bundle_ok ($system_bundle)"
    echo "[CERTS-SVC-DIAG] system_ca_chain: $system_ca_ok ($system_ca_chain or $system_ca_chain_alt)"
...
copy_certificates_from_vault_agent() {
    ...
    echo "[CERTS-COPY-VA] ========================================"
    echo "[CERTS-COPY-VA] Копирование сертификатов vault-agent"
    echo "[CERTS-COPY-VA] ========================================"
    
    # Проверяем наличие сертификатов в /opt/vault/certs/
    if [[ ! -f "/opt/vault/certs/server_bundle.pem" ]]; then
        print_error "Сертификаты не найдены в /opt/vault/certs/"
        echo "[CERTS-COPY-VA] ❌ /opt/vault/certs/server_bundle.pem не найден"
        echo "[CERTS-COPY-VA] Возможно, vault-agent еще не сгенерировал сертификаты"
        echo "[CERTS-COPY-VA] Проверьте: sudo systemctl status vault-agent"
        echo "[CERTS-COPY-VA] Логи: sudo journalctl -u vault-agent -n 50"
        return 1
    fi
    
    echo "[CERTS-COPY-VA] ✅ Сертификаты найдены в /opt/vault/certs/"
    ...
    echo "[CERTS-COPY-VA] Копирование server_bundle.pem..."
    if cp /opt/vault/certs/server_bundle.pem "$vault_certs_dir/server_bundle.pem" 2>/dev/null; then
        ...
        echo "[CERTS-COPY-VA] ✅ server_bundle.pem скопирован ($size байт)"
    else
    ...
    echo "[CERTS-COPY-VA] Копирование ca_chain.crt..."
    if cp /opt/vault/certs/ca_chain.crt "$vault_certs_dir/ca_chain.crt" 2>/dev/null; then
        ...
        echo "[CERTS-COPY-VA] ✅ ca_chain.crt скопирован ($size байт)"
    else
        print_warning "ca_chain.crt не найден (опционально)"
    fi
    
    # Копируем grafana-client.pem
    echo "[CERTS-COPY-VA] Копирование grafana-client.pem..."
    if cp /opt/vault/certs/grafana-client.pem "$grafana_certs_dir/grafana-client.pem" 2>/dev/null; then
        ...
        echo "[CERTS-COPY-VA] ✅ grafana-client.pem скопирован ($size байт)"
    else
        print_warning "grafana-client.pem не найден (опционально)"
    fi
    
    echo "[CERTS-COPY-VA] ========================================"
    echo "[CERTS-COPY-VA] ✅ Сертификаты успешно скопированы"
    echo "[CERTS-COPY-VA] ========================================"
...
get_certificates_from_jenkins() {
    ...
    echo "[CERTS-EXTRACT] ========================================"
    echo "[CERTS-EXTRACT] Извлечение сертификатов из Jenkins"
    echo "[CERTS-EXTRACT] ========================================"
    ...
    if [[ -z "$cred_json_path" || ! -f "$cred_json_path" ]]; then
        ...
        echo "[CERTS-EXTRACT] ❌ temp_data_cred.json не найден"
        return 1
    fi
    
    echo "[CERTS-EXTRACT] Используется temp_data_cred.json: $cred_json_path"
    ...
    if ! jq -e '.certificates' "$cred_json_path" >/dev/null 2>&1; then
        ...
        echo "[CERTS-EXTRACT] ⚠️  Секция 'certificates' отсутствует"
        echo "[CERTS-EXTRACT] Возможно, сертификаты не были сгенерированы в Jenkins"
        return 1
    fi
    
    echo "[CERTS-EXTRACT] ✅ Секция 'certificates' найдена"
    ...
    echo "[CERTS-EXTRACT] Извлечение server_bundle.pem..."
    ...
    if [[ ! -s "$vault_certs_dir/server_bundle.pem" ]]; then
        ...
        echo "[CERTS-EXTRACT] ⚠️  server_bundle.pem пустой"
        return 1
    fi
    
    chmod 600 "$vault_certs_dir/server_bundle.pem"
    echo "[CERTS-EXTRACT] ✅ server_bundle.pem извлечен ($(stat -c%s "$vault_certs_dir/server_bundle.pem" 2>/dev/null || echo "0") байт)"
    
    # Извлекаем ca_chain.crt
    echo "[CERTS-EXTRACT] Извлечение ca_chain.crt..."
    ...
    if [[ ! -s "$vault_certs_dir/ca_chain.crt" ]]; then
        ...
        echo "[CERTS-EXTRACT] ⚠️  ca_chain.crt пустой"
    else
        chmod 600 "$vault_certs_dir/ca_chain.crt"
        echo "[CERTS-EXTRACT] ✅ ca_chain.crt извлечен ($(stat -c%s "$vault_certs_dir/ca_chain.crt" 2>/dev/null || echo "0") байт)"
    fi
    
    # Извлекаем grafana-client.pem
    echo "[CERTS-EXTRACT] Извлечение grafana-client.pem..."
    ...
    if [[ ! -s "$grafana_certs_dir/grafana-client.pem" ]]; then
        ...
        echo "[CERTS-EXTRACT] ⚠️  grafana-client.pem пустой"
    else
        chmod 600 "$grafana_certs_dir/grafana-client.pem"
        echo "[CERTS-EXTRACT] ✅ grafana-client.pem извлечен ($(stat -c%s "$grafana_certs_dir/grafana-client.pem" 2>/dev/null || echo "0") байт)"
    fi
    
    echo "[CERTS-EXTRACT] ========================================"
    echo "[CERTS-EXTRACT] ✅ Все сертификаты извлечены из temp_data_cred.json"
    echo "[CERTS-EXTRACT] ========================================"
    ...
    if command -v openssl >/dev/null 2>&1 && [[ -s "$vault_certs_dir/server_bundle.pem" ]]; then
        echo "[CERTS-EXTRACT] Информация о сертификате:"
        openssl x509 -in "$vault_certs_dir/server_bundle.pem" -noout -text 2>/dev/null | grep -E "Subject:|Not Before|Not After|DNS:" || echo "[CERTS-EXTRACT] (не удалось прочитать сертификат)"
    fi
...
distribute_certificates_to_services() {
    ...
    echo "[CERTS-DIST] ========================================"
    echo "[CERTS-DIST] Копирование сертификатов в конфиги"
    echo "[CERTS-DIST] ========================================"
    ...
    if [[ -d "$prom_conf_dir" ]]; then
        if [[ -f "$vault_bundle" ]]; then
            ...
            echo "[CERTS-DIST] ✅ Prometheus: server_bundle.pem"
        fi
        
        if [[ -f "$ca_chain" ]]; then
            cp "$ca_chain" "$prom_conf_dir/ca_chain.crt"
            chmod 600 "$prom_conf_dir/ca_chain.crt"
            echo "[CERTS-DIST] ✅ Prometheus: ca_chain.crt"
        fi
    fi
    ...
    if [[ -d "$grafana_conf_dir" ]]; then
        if [[ -f "$vault_bundle" ]]; then
            ...
            echo "[CERTS-DIST] ✅ Grafana: server_bundle.pem"
        fi
        ...
        if [[ -f "$grafana_client" ]]; then
            ...
            echo "[CERTS-DIST] ✅ Grafana: grafana-client.pem"
        fi
    fi
    ...
    if [[ -d "$harvest_conf_dir" ]]; then
        if [[ -f "$ca_chain" ]]; then
            ...
            echo "[CERTS-DIST] ✅ Harvest: ca_chain.crt"
        fi
    fi
    
    echo "[CERTS-DIST] ✅ Сертификаты распределены по всем сервисам"
    echo "[CERTS-DIST] ========================================"
}
...
restart_services_for_certificates() {
    ...
    echo "[CERTS-RESTART] ========================================"
    echo "[CERTS-RESTART] Перезапуск сервисов"
    echo "[CERTS-RESTART] ========================================"
    
    for service in "${services[@]}"; do
        echo "[CERTS-RESTART] Перезапуск $service.service..."
        
        if sudo -n -u "$mon_sys_user" env "$xdg_env" /usr/bin/systemctl --user restart "$service.service" 2>/dev/null; then
            echo "[CERTS-RESTART] ✅ $service.service перезапущен"
            
            # Проверяем статус
            if sudo -n -u "$mon_sys_user" env "$xdg_env" /usr/bin/systemctl --user is-active "$service.service" >/dev/null 2>&1; then
                echo "[CERTS-RESTART] ✅ $service.service активен"
            else
                echo "[CERTS-RESTART] ⚠️  $service.service не активен после перезапуска"
            fi
        else
            echo "[CERTS-RESTART] ❌ Не удалось перезапустить $service.service"
        fi
    done
    
    echo "[CERTS-RESTART] ✅ Перезапуск сервисов завершен"
    echo "[CERTS-RESTART] ========================================"
}
...
renew_certificates_only() {
    ...
    echo "[CERT-RENEW] ========================================"
    echo "[CERT-RENEW] Certificate Renewal Mode"
    echo "[CERT-RENEW] ========================================"
    ...
    echo "[CERT-RENEW] ========================================"
    echo "[CERT-RENEW] ✅ Обновление сертификатов завершено"
    echo "[CERT-RENEW] ========================================"
...
main() {
    ...
    echo "========================================"
    echo "[MAIN] START: main() функция запущена"
    echo "[MAIN] Время: $(date)"
    echo "[MAIN] PWD: $(pwd)"
    echo "[MAIN] User: $(whoami)"
    echo "========================================"
    
    log_message "=== Начало развертывания мониторинговой системы ${DEPLOY_VERSION} ==="
    ensure_working_directory
    
    echo "[MAIN] Calling print_header..."
    print_header
    echo "[MAIN] print_header completed"
    
    echo "[MAIN] Calling init_diagnostic_log..."
    # Инициализация diagnostic log
    init_diagnostic_log
    ...
    init_debug_log
    echo "[MAIN] init_debug_log completed"
    
    echo "[MAIN] Setting up trap DEBUG..."
    ...
    echo "[MAIN] trap DEBUG set"
    
    echo "[MAIN] Calling log_debug_extended..."
    log_debug "=== DEPLOYMENT STARTED ==="
    log_debug_extended
    echo "[MAIN] log_debug_extended completed"
    ...
    if [[ "${RENEW_CERTIFICATES_ONLY:-false}" == "true" ]]; then
        echo "[MAIN] ========================================"
        echo "[MAIN] РЕЖИМ: Обновление только сертификатов"
        echo "[MAIN] ========================================"
        ...
        echo "[MAIN] ========================================"
        echo "[MAIN] ✅ Certificate renewal завершен"
        echo "[MAIN] ========================================"
    ...
    if [[ "${RLM_PHASE_ONLY:-false}" == "true" ]]; then
        echo "[MAIN] ========================================"
        echo "[MAIN] РЕЖИМ: RLM_PHASE_ONLY (только RPM фаза)"
        echo "[MAIN] RLM_PACKAGE_FILTER=${RLM_PACKAGE_FILTER:-<не задан>}"
        echo "[MAIN] ========================================"
    ...
    echo "[MAIN] ✅ ensure_monitoring_users_in_as_admin completed"
    log_debug "Completed: ensure_monitoring_users_in_as_admin"
    
    echo "[MAIN] Вызов ensure_mon_sys_in_grafana_group..."
    log_debug "Calling: ensure_mon_sys_in_grafana_group"
    ensure_mon_sys_in_grafana_group || {
        echo "[MAIN] ⚠️  ensure_mon_sys_in_grafana_group FAILED, continuing..."
    ...
    echo "[MAIN] ✅ ensure_mon_sys_in_grafana_group completed"
    log_debug "Completed: ensure_mon_sys_in_grafana_group"
    
    echo "[MAIN] Вызов cleanup_all_previous..."
    log_debug "Calling: cleanup_all_previous"
    cleanup_all_previous || {
        echo "[MAIN] ⚠️  cleanup_all_previous FAILED, continuing..."
    ...
    echo "[MAIN] ✅ cleanup_all_previous completed"
    log_debug "Completed: cleanup_all_previous"
    
    echo "[MAIN] Вызов create_directories..."
    log_debug "Calling: create_directories"
    create_directories || {
        echo "[MAIN] ⚠️  create_directories FAILED, continuing..."
    ...
    echo "[MAIN] ✅ create_directories completed"
    ...
    if [[ "${USE_SIMPLIFIED_CERT_FLOW:-false}" == "true" ]]; then
        ...
        echo "[MAIN] ✅ USE_SIMPLIFIED_CERT_FLOW=true: включен non-root/simplified путь"
        log_debug "USE_SIMPLIFIED_CERT_FLOW=true: using simplified certificate flow"

        echo "[MAIN] ========================================"
        echo "[MAIN] Вызов get_certificates_from_jenkins..."
        log_debug "Calling: get_certificates_from_jenkins"

        get_certificates_from_jenkins

        echo "[MAIN] ✅ get_certificates_from_jenkins завершена успешно"
        ...
        echo "[MAIN] ⚠️  USE_SIMPLIFIED_CERT_FLOW=false: используется legacy vault-agent путь"
    ...
    if [[ "${SKIP_VAULT_INSTALL:-false}" == "true" ]]; then
        ...
        echo "[MAIN] ⚠️  SKIP_VAULT_INSTALL=true: пропускаем install_vault_via_rlm"
        ...
        echo "[MAIN] Вызов install_vault_via_rlm..."
        log_debug "Calling: install_vault_via_rlm"
        
        install_vault_via_rlm
        
        echo "[MAIN] ✅ install_vault_via_rlm завершена успешно"
    ...
    echo "[MAIN] ========================================"
    echo "[MAIN] Вызов setup_vault_config..."
    log_debug "Calling: setup_vault_config"
    
    setup_vault_config
    
    echo "[MAIN] ✅ setup_vault_config завершена успешно"
    log_debug "Completed: setup_vault_config"
    write_diagnostic "setup_vault_config выполнена"
    
    echo "[MAIN] ========================================"
    echo "[MAIN] Вызов copy_certificates_from_vault_agent..."
    log_debug "Calling: copy_certificates_from_vault_agent"
    
    copy_certificates_from_vault_agent || {
        echo "[MAIN] ⚠️  copy_certificates_from_vault_agent FAILED"
    ...
    if [[ "${SKIP_RPM_INSTALL:-false}" == "true" ]]; then
        ...
        echo "[MAIN] Вызов create_rlm_install_tasks..."
        log_debug "Calling: create_rlm_install_tasks"
        create_rlm_install_tasks
        echo "[MAIN] ✅ create_rlm_install_tasks завершена успешно"
    ...
    echo "[MAIN] ========================================"
    echo "[MAIN] Вызов setup_certificates_after_install..."
    log_debug "Calling: setup_certificates_after_install"
    setup_certificates_after_install
    echo "[MAIN] ✅ setup_certificates_after_install завершена успешно"
    log_debug "Completed: setup_certificates_after_install"
    
    echo "[MAIN] Вызов configure_harvest..."
