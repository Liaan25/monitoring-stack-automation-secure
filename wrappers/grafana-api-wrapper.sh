...
#   health_check       <grafana_url>                 (проверка /api/health)
...
main() {
  case "$MODE" in
    ...
    health_check)
      local url="$2"
      validate_grafana_url "$url"
      # НЕ требуется токен для health endpoint - это публичный endpoint
      curl -k -s -o /dev/null -w "%{http_code}" "$url/api/health" || echo "000"
      ;;
