...
## [4.12.3-cert-wait-fix] - 2026-02-02

### ⏳ Добавлено ожидание создания сертификатов vault-agent

**ПРОБЛЕМА**: 
```
[CERTS-COPY] ❌ vault_bundle пустой: /home/CI10742292-lnx-mon_ci/monitoring/certs/vault/server_bundle.pem
```

**ПРИЧИНА**: 
Vault-agent **создает файл но он ПУСТОЙ** пока сертификаты не сгенерированы! Процесс:
1. Vault-agent устанавливается (RLM задача завершена)
2. Vault-agent запускается как сервис
3. Vault-agent создает пустой файл `/opt/vault/certs/server_bundle.pem`
4. Vault-agent подключается к Vault серверу (может занять время)
5. Vault-agent запрашивает сертификаты
6. Vault-agent заполняет файл сертификатами

**Наш скрипт не ждал шаги 4-6**! Он видел что файл существует (шаг 3) и пытался его обработать.

**РЕШЕНИЕ**: 
1. ✅ **Добавлено ожидание в `setup_certificates_after_install()`**:
   - Ждем пока `/opt/vault/certs/server_bundle.pem` не станет **НЕПУСТЫМ** (>0 байт)
   - Максимум 5 минут (60 попыток × 5 секунд)
   - Логируем прогресс

2. ✅ **Улучшены сообщения об ошибках**:
   - Если файл пустой → понятное объяснение что делать
   - Диагностика: проверка оригинального файла, логов vault-agent, конфигурации

3. ✅ **Логирование размера файла**:
   - Показываем размер файла при успешном обнаружении
...
1. **`setup_certificates_after_install()`** (строки ~3196-3255):
   - Добавлен цикл ожидания (до 5 минут)
   - Проверка что файл не пустой (`-s` вместо `-f`)
   - Логирование прогресса и размера файла

2. **`copy_certs_to_user_dirs()`** (строки ~2447-2475):
...
```
ДО (падало):
1. Проверяем: if [[ -f "/opt/vault/certs/server_bundle.pem" ]]
2. Файл существует (но пустой) → OK
3. Копируем пустой файл в user-space
4. Пытаемся обработать → ОШИБКА "файл пустой"

ПОСЛЕ (работает):
1. Проверяем: while [[ ! -s "/opt/vault/certs/server_bundle.pem" ]]
2. Ждем пока файл не станет непустым (до 5 минут)
3. Если через 5 минут все еще пустой → ошибка с диагностикой
4. Копируем НЕПУСТОЙ файл в user-space
5. Обрабатываем успешно
```
...
**Теперь будет**:
```
[CERTS] Ожидание создания сертификатов vault-agent...
[CERTS] ⏳ Файл существует но пустой (0 байт), ждем... (1/60)
[CERTS] ⏳ Файл существует но пустой (0 байт), ждем... (2/60)
[CERTS] ⏳ Файл существует но пустой (0 байт), ждем... (3/60)
[CERTS] ✅ Найден НЕПУСТОЙ системный vault bundle: /opt/vault/certs/server_bundle.pem
[CERTS]   Размер: 4096 байт, попытка: 4/60
```

**Или если проблема**:
```
[CERTS] ❌ ТАЙМАУТ: vault-agent не создал сертификаты за 300 секунд
[ERROR] Vault-agent не создал сертификаты. Проверьте:
[ERROR] 1. Логи vault-agent: journalctl -u vault-agent
[ERROR] 2. Состояние сервиса: systemctl status vault-agent
[ERROR] 3. Конфигурацию: /opt/vault/conf/agent.hcl
```

---

### Fixed
- ✅ Добавлено ожидание создания сертификатов vault-agent (до 5 минут)
- ✅ Улучшена диагностика при пустых файлах сертификатов
- ✅ Логирование размера файлов для отладки
