...
- ✅ **Автоматическая интеграция в `setup_certificates_after_install()`**:
  ...
  - После успеха: использует workaround через `sys_user` (изменения группы требуют новой сессии)
  - **Попытка 2**: Если RLM не сработал → копирование через `sudo -u sys_user` (fallback)
...
2. ПОПЫТКА 1: Автоматическое добавление в va-read
   ...
   ├─ Успех? → Копируем через sys_user workaround ✅
   └─ Неудача? → Идем дальше ⬇️

3. ПОПЫТКА 2: Копирование через sys_user
   ├─ Проверяем: sys_user в группе va-read?
   ├─ Да? → sudo -u sys_user cp ... ✅
   └─ Нет? → Идем дальше ⬇️
...
- `🔐 User: CI10742292-lnx-mon_ci │ Попытка 1/60 │ Статус: in_progress 🔄`
- `✅ Пользователь CI10742292-lnx-mon_ci добавлен в группу CI10742292-lnx-va-read за 2.3м`
- `[CERTS] ✅ Bundle скопирован через sys_user workaround`
...
**ПРОБЛЕМА**:
- Сертификаты в `/opt/vault/certs/` принадлежат пользователю `CI10742292-lnx-va-start` и группе `CI10742292-lnx-va-read`
- Права: `rw-------` (600) - только владелец может читать
- `CI10742292-lnx-mon_ci` (CI-user) **НЕ СОСТОИТ** в группе `va-read` → **нет доступа** ❌
- `CI10742292-lnx-mon_sys` (sys-user) **СОСТОИТ** в группе `va-read` → **есть доступ** ✅
...
- ✅ **Проверка прав доступа** перед копированием:
  ...
  2. Если нет доступа → проверяем, есть ли доступ у sys-user через группу `va-read`
  3. Если sys-user в `va-read` → копируем через `sudo -u ${KAE}-lnx-mon_sys`
...
- ✅ **Понятные ошибки**:
  ...
  ERROR: Права на файл: -rw------- 1 CI10742292-lnx-va-start CI10742292-lnx-va-read ...
  ERROR: 
  ERROR: ТРЕБУЕТСЯ: Добавить CI10742292-lnx-mon_ci в группу CI10742292-lnx-va-read
  ERROR: Используйте RLM или IDM для добавления пользователя в группу
  ```

**Решение**:
1. **Предпочтительный вариант**: Добавить `${KAE}-lnx-mon_ci` в группу `${KAE}-lnx-va-read` через RLM/IDM (одноразово)
