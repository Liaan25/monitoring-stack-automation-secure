...
### 🔧 Fixed
- **Jenkins Credential**: Исправлена ошибка `Could not find credentials entry with ID 'vault-token'`
  - Добавлен новый параметр `VAULT_CREDENTIAL_ID` (по умолчанию: `vault-agent-dev`)
  - Добавлена обработка ошибок (try-catch) при получении секретов из Vault
  - Улучшены сообщения об ошибках с инструкциями по созданию credential

### 📝 Changed
- Обновлена документация с инструкциями по настройке Jenkins credentials
