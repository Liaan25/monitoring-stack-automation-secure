...
ssh ${SSH_OPTS} "${SSH_USER}@${TARGET_SERVER}" \
  ...
  PROMETHEUS_LOCAL_INSTALL_FROM_ARCHIVE="${PROMETHEUS_LOCAL_INSTALL_FROM_ARCHIVE:-true}" \
  DOWNLOAD_CHECK_ENABLED="${DOWNLOAD_CHECK_ENABLED:-false}" \
  SKIP_RLM_ADD_USER_GROUP_TASK="${SKIP_RLM_ADD_USER_GROUP_TASK:-false}" \
